...
**Flex-item auto min-size (CSS §4.5 item-side, shipped under CSS preset)**: CSS §4.5 has two complementary rules. Flexily now implements both — the *container* side (overflow containers can shrink to 0) and the *item* side (flex items default to a content-based minimum, not 0). Under Yoga preset (`flexShrink: 0` + `min: undefined → 0`) this is invisible because items never shrink. Under CSS preset (`flexShrink: 1` + `min: auto → content`), items keep their intrinsic content size when overflow is visible, and shrink to 0 when overflow is hidden/scroll/auto. The implementation derives content-size separately from `flex-basis` so `flex: 1 1 0` patterns also keep their content. See `tests/auto-min-size.test.ts`. **Remaining approximation gaps** (smaller, deferred): wrapping row text uses max-content not min-content; nodes-with-children + definite flex-basis fall back to `baseSize`; aspect-ratio transferred sizes not folded in.
